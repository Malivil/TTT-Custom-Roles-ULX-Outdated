...
function UpdateRoles()
    ...
    table.insert(ulx.target_role, "detraitor") -- Add "detective" to the table.
    ...
    table.insert(ulx.target_role, "zombie") -- Add "innocent" to the table.
    table.insert(ulx.target_role, "vampire") -- Add "innocent" to the table.
    table.insert(ulx.target_role, "swapper") -- Add "innocent" to the table.
    table.insert(ulx.target_role, "assassin") -- Add "innocent" to the table.
...
function GetRoleStartingCredits(role)
    return (role == ROLE_TRAITOR and GetConVar("ttt_credits_starting"):GetFloat()) or
        (role == ROLE_DETECTIVE and GetConVar("ttt_det_credits_starting"):GetFloat()) or
        (role == ROLE_MERCENARY and GetConVar("ttt_mer_credits_starting"):GetFloat()) or
        (role == ROLE_KILLER and GetConVar("ttt_kil_credits_starting"):GetFloat()) or
        (role == ROLE_ASSASSIN and GetConVar("ttt_asn_credits_starting"):GetFloat()) or
        (role == ROLE_HYPNOTIST and GetConVar("ttt_hyp_credits_starting"):GetFloat()) or
        (role == ROLE_ZOMBIE and GetConVar("ttt_zom_credits_starting"):GetFloat()) or
        (role == ROLE_VAMPIRE and GetConVar("ttt_vam_credits_starting"):GetFloat()) or
        (role == ROLE_DETRAITOR and GetConVar("ttt_der_credits_starting"):GetFloat()) or 0
end
...
--[Slay Next Round]---------------------------------------------------------------------------------
--[[ulx.slaynr][Slays <target(s)> for the next next <num_slay> round(s).]
@param  {[PlayerObject]} calling_ply   [The player who used the command.]
@param  {[PlayerObject]} target_plys   [The player(s) who will have the effects of the command applied to them.]
@param  {[Number]}       num_slay      [The number of rounds the target(s) will be slayed for.]
@param  {[Boolean]}      should_slaynr [Whether the target(s) should be slayed the next round.]
--]]
...
function ulx.force(calling_ply, target_plys, target_role, should_silent)
    if not GetConVarString("gamemode") == "terrortown" then ULib.tsayError(calling_ply, gamemode_error, true) else
        ...
        if target_role == "traitor" or target_role == "t" then role, role_grammar, role_string, role_credits = ROLE_TRAITOR, "a ", "traitor", GetRoleStartingCredits(ROLE_TRAITOR) end
        if target_role == "detective" or target_role == "d" then role, role_grammar, role_string, role_credits = ROLE_DETECTIVE, "a ", "detective", GetRoleStartingCredits(ROLE_DETECTIVE) end
        if target_role == "mercenary" or target_role == "m" then role, role_grammar, role_string, role_credits = ROLE_MERCENARY, "a ", "mercenary", GetRoleStartingCredits(ROLE_MERCENARY) end
        if target_role == "hypnotist" or target_role == "h" then role, role_grammar, role_string, role_credits = ROLE_HYPNOTIST, "a ", "hypnotist", GetRoleStartingCredits(ROLE_HYPNOTIST) end
        if target_role == "glitch" or target_role == "g" then role, role_grammar, role_string, role_credits = ROLE_GLITCH, "a ", "glitch", GetRoleStartingCredits(ROLE_GLITCH) end
        if target_role == "jester" or target_role == "j" then role, role_grammar, role_string, role_credits = ROLE_JESTER, "a ", "jester", GetRoleStartingCredits(ROLE_JESTER) end
        if target_role == "phantom" or target_role == "p" then role, role_grammar, role_string, role_credits = ROLE_PHANTOM, "a ", "phantom", GetRoleStartingCredits(ROLE_PHANTOM) end
        if target_role == "zombie" or target_role == "z" then role, role_grammar, role_string, role_credits = ROLE_ZOMBIE, "a ", "zombie", GetRoleStartingCredits(ROLE_ZOMBIE) end
        if target_role == "vampire" or target_role == "v" then role, role_grammar, role_string, role_credits = ROLE_VAMPIRE, "a ", "vampire", GetRoleStartingCredits(ROLE_VAMPIRE) end
        if target_role == "swapper" or target_role == "s" then role, role_grammar, role_string, role_credits = ROLE_SWAPPER, "a ", "swapper", GetRoleStartingCredits(ROLE_SWAPPER) end
        if target_role == "assassin" or target_role == "a" then role, role_grammar, role_string, role_credits = ROLE_ASSASSIN, "an ", "assassin", GetRoleStartingCredits(ROLE_ASSASSIN) end
        if target_role == "killer" or target_role == "k" then role, role_grammar, role_string, role_credits = ROLE_KILLER, "a ", "killer", GetRoleStartingCredits(ROLE_KILLER) end
        if target_role == "innocent" or target_role == "i" then role, role_grammar, role_string, role_credits = ROLE_INNOCENT, "an ", "innocent", GetRoleStartingCredits(ROLE_INNOCENT) end
        if target_role == "detraitor" or target_role == "dr" then role, role_grammar, role_string, role_credits = ROLE_DETRAITOR, "a ", "detraitor", GetRoleStartingCredits(ROLE_DETRAITOR) end
...
local function updateNextround()
    ...
    table.insert(ulx.next_round, "detraitor") -- Add "detraitor" to the table.
    ...
    table.insert(ulx.next_round, "zombie") -- Add "innocent" to the table.
    table.insert(ulx.next_round, "vampire") -- Add "innocent" to the table.
    table.insert(ulx.next_round, "swapper") -- Add "innocent" to the table.
    table.insert(ulx.next_round, "assassin") -- Add "innocent" to the table.
...
function ulx.nextround(calling_ply, target_plys, next_round)
    if not GetConVarString("gamemode") == "terrortown" then ULib.tsayError(calling_ply, gamemode_error, true) else
        ...
        for i = 1, #target_plys do
            ...
            if next_round == "detraitor" then
                if PlysMarkedForTraitor[ID] == true or PlysMarkedForDetective[ID] == true or PlysMarkedForDetraitor[ID] == true or PlysMarkedForMercenary[ID] == true or PlysMarkedForHypnotist[ID] == true or PlysMarkedForGlitch[ID] == true or PlysMarkedForJester[ID] == true or PlysMarkedForPhantom[ID] == true or PlysMarkedForZombie[ID] == true or PlysMarkedForVampire[ID] == true or PlysMarkedForSwapper[ID] == true or PlysMarkedForAssassin[ID] == true or PlysMarkedForKiller[ID] == true or PlysMarkedForInnocent[ID] == true then
                    ULib.tsayError(calling_ply, "that player is already marked for the next round!", true)
                else
                    PlysMarkedForDetraitor[ID] = true
                    table.insert(affected_plys, v)
                end
            end
            if next_round == "unmark" then
                ...
                if PlysMarkedForDetraitor[ID] == true then
                    PlysMarkedForDetraitor[ID] = false
                    table.insert(affected_plys, v)
                end
            end
        end
...
local function TraitorMarkedPlayers()
    for k, v in pairs(PlysMarkedForTraitor) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_TRAITOR))
...
local function DetectiveMarkedPlayers()
    for k, v in pairs(PlysMarkedForDetective) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_DETECTIVE))
...
local function DetraitorMarkedPlayers()
    for k, v in pairs(PlysMarkedForDetraitor) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_DETRAITOR))
            PlysMarkedForDetraitor[k] = false
            if ply:HasWeapon("weapon_hyp_brainwash") then
                ply:StripWeapon("weapon_hyp_brainwash")
            end
            if ply:HasWeapon("weapon_vam_fangs") then
...
hook.Add("TTTSelectRoles", "Admin_Round_Detraitor", DetraitorMarkedPlayers)
...
local function MercenaryMarkedPlayers()
    for k, v in pairs(PlysMarkedForMercenary) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_MERCENARY))
...
local function HypnotistMarkedPlayers()
    for k, v in pairs(PlysMarkedForHypnotist) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_HYPNOTIST))
...
local function GlitchMarkedPlayers()
    for k, v in pairs(PlysMarkedForGlitch) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_GLITCH))
...
local function JesterMarkedPlayers()
    for k, v in pairs(PlysMarkedForJester) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_JESTER))
...
local function PhantomMarkedPlayers()
    for k, v in pairs(PlysMarkedForPhantom) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_PHANTOM))
...
local function ZombieMarkedPlayers()
    for k, v in pairs(PlysMarkedForZombie) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_ZOMBIE))
...
local function VampireMarkedPlayers()
    for k, v in pairs(PlysMarkedForVampire) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_VAMPIRE))
...
local function SwapperMarkedPlayers()
    for k, v in pairs(PlysMarkedForSwapper) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_SWAPPER))
...
local function AssassinMarkedPlayers()
    for k, v in pairs(PlysMarkedForAssassin) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_ASSASSIN))
...
local function KillerMarkedPlayers()
    for k, v in pairs(PlysMarkedForKiller) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_KILLER))
...
local function InnocentMarkedPlayers()
    for k, v in pairs(PlysMarkedForInnocent) do
        if v then
            ...
            ply:AddCredits(GetRoleStartingCredits(ROLE_INNOCENT))
